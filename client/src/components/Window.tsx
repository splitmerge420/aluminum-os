...
import { Minus, Square, X, Maximize2, Terminal, FolderOpen, Mail, CalendarDays, Brain, Settings, Globe, StickyNote, Activity, Shield, Database, Archive, GitBranch, ScrollText, Cpu, TerminalSquare, Workflow, DollarSign, Hexagon, HeartPulse } from "lucide-react";
...
export default function Window({ window: win, children }: WindowProps) {
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      const newY = Math.max(32, dragRef.current.winY + dy); // Don't go above topbar
      moveWindow(win.id, newX, newY);

      // Snap hints
      if (e.clientX <= 2) setShowSnapHint("left");
      else if (e.clientX >= window.innerWidth - 2) setShowSnapHint("right");
      else if (e.clientY <= 2) setShowSnapHint("top");
  ...
  return (
    <>
      ...
      {showSnapHint && (
        <div
          className="fixed z-[99990] border-2 border-cyan-400/40 rounded-xl bg-cyan-400/5 transition-all duration-200"
          style={{
            top: showSnapHint === "top" ? 32 : 32,
            left: showSnapHint === "right" ? "50%" : 0,
            width: showSnapHint === "top" ? "100vw" : "50vw",
            height: showSnapHint === "top" ? "calc(100vh - 32px - 80px)" : "calc(100vh - 32px - 80px)",
          }}
        />
      )}
      ...
      >
        ...
        {!win.isMaximized && (
          <>
            <div className={`${resizeEdgeClass} top-0 left-2 right-2 h-1 cursor-n-resize`} onMouseDown={e => handleResizeStart(e, "n")} />
            <div className={`${resizeEdgeClass} bottom-0 left-2 right-2 h-1 cursor-s-resize`} onMouseDown={e => handleResizeStart(e, "s")} />
            <div className={`${resizeEdgeClass} left-0 top-2 bottom-2 w-1 cursor-w-resize`} onMouseDown={e => handleResizeStart(e, "w")} />
            <div className={`${resizeEdgeClass} right-0 top-2 bottom-2 w-1 cursor-e-resize`} onMouseDown={e => handleResizeStart(e, "e")} />
        ...
        >
          ...
          <div className="flex items-center gap-1.5 no-drag mr-3">
            <button
        ...
        {/* Window content */}
        <div className="glass overflow-auto" style={{ height: "calc(100% - 40px)" }}>
          {children}
        </div>
      </motion.div>
    </>
